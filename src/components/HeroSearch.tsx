            ...
            {results.length > 0 ? (
               <div>
                 <div className="px-4 py-3 bg-gray-50/50 dark:bg-slate-900/50 border-b border-gray-100 dark:border-slate-700">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Top Matches</p>
                 </div>
                 {results.slice(0, 5).map((item, i) => (
                   ...
                   >
                     <div className="flex items-center gap-4">
                       ...
                       <div>
                         <p className="font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{item.title}</p>
                         <p className="text-xs text-gray-500 dark:text-gray-400">{item.category} • {item.tags.slice(0, 2).join(', ')}</p>
                       </div>
                     </div>
